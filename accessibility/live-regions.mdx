...
<button onclick="window.LiveRegionSystem.announceStatus('Operation completed successfully')">
  Complete Operation
</button>
...
```tsx
import { useLiveRegion, useStatusAnnouncer } from '@/components/accessibility/LiveRegionContext';
...
```ts
import { announceStatus, announceAlert, log, announceProgress } from '@/utils/liveRegion';
