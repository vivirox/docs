...
class AuditLoggingService {
  ...
  async export(format: ExportFormat, criteria: LogQueryCriteria): Promise<Blob> {
...
async function analyzeSecurityEvents(logService: AuditLoggingService, timeframe: TimeRange) {
  ...
  return {
    ...
    riskScore: calculateRiskScore(failedLogins, sensitiveAccess, offHoursActivity)
  };
}
```
...
const auditLogger = new AuditLoggingService({
  ...
  alerting: {
    ...
    criticalEvents: ['unauthorized-access', 'integrity-violation', 'excessive-failures']
  },
  securityConfig: {
    ...
    redactionRules: [
      ...
      { target: 'actor.ipAddress', strategy: 'partial', pattern: /(\d+)\.(\d+)\.(\d+)\.(\d+)/, replacement: '$1.$2.XXX.XXX' }
    ],
    accessControls: [
...
async function generateComplianceReport(
  ...
  const accessByTarget = groupBy(phiAccessEvents, (event) => event.action.target);
  const failedAccessAttempts = phiAccessEvents.filter(e => e.action.status === 'failure');
