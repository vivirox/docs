...
function generateCacheKey(params: {
  ...
  const timeString = params.timeRange ? `:${params.timeRange[0].getTime()}-${params.timeRange[1].getTime()}` : '';
...
const cacheManager = new SessionDataCacheManager({
  layers: [
    { name: 'memory', priority: 1, capacity: 50, ttl: 300000, securityLevel: 'medium', compressionEnabled: false, encryptionEnabled: false },
    { name: 'indexedDB', priority: 2, capacity: 200, ttl: 3600000, securityLevel: 'high', compressionEnabled: true, encryptionEnabled: true }
  ],
