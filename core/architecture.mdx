...
<Frame>
  ``` mermaid sequenceDiagram participant Client participant API participant
  Auth participant DB Client->>API: Login Request API->>Auth: Validate
  Credentials Auth->>DB: Check User DB-->>Auth: User Data Auth-->>API: JWT Token
...
<Frame>
  ``` mermaid graph LR Input[User Input] --> Preprocess[Preprocessing]
  Preprocess --> Models[AI Models] Models --> Analysis[Analysis] Models -->
